...
const ResMenuItem = ({ item, resName }) => {
  ...
  const currentCartstate =
    cartState && Object.keys(cartState).length
      ? cartState[Object.keys(cartState)[0]]
      : {};
  ...
  return (
    <div className="resMenuMainCon">
      <div className="resMenuLeft">
        ...
        <Typography
          sx={{ fontWeight: "bold", fontSize: "small", marginBottom: "8px" }}
        >
      ...
      <div className="resMenuRight">
        <img
          src={resmenuImgUrl + item?.card?.info?.imageId}
          alt="food img"
          className="resFoodImg"
        />
        <Button
          className="resFoodAddBtn"
          variant="contained"
          onClick={(e) => AddCartItem(e, { ...item?.card?.info })}
        >
          Add{" "}
          {currentCartstate &&
          Object.keys(currentCartstate).length &&
          Object.keys(currentCartstate).includes(item?.card?.info?.id)
            ? currentCartstate[item?.card?.info?.id]?.count
            : ""}
        </Button>
      </div>
    </div>
